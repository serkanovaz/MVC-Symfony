...
class ShopcartRepository extends ServiceEntityRepository
{
    ...
    public function totalamount($id):float{
    ...
    public function shopcartproducts($id):array{

        $em = $this->getEntityManager();
        $query = $em->createQuery('SELECT p.keywords,p.sprice,  s.id,s.quantity,s.productid,s.userid, (p.sprice * s.quantity) as total
